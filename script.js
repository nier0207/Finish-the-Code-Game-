...
// HINTS PER LANGUAGE - separate hints for each language
let hintsRemaining = {
    java: 3,
    csharp: 3,
    web: 3
...
// Language info
const languageInfo = {
    java: { name: 'Java', color: 'java' },
    csharp: { name: 'C#', color: 'csharp' },
    web: { name: 'Web Dev', color: 'web' }
};

// Storage keys
const HINTS_STORAGE_KEY = 'finish_code_hints_per_language';
...
// Load hints for all languages from localStorage
function loadAllHints() {
    const saved = localStorage.getItem(HINTS_STORAGE_KEY);
    if (saved) {
        const loaded = JSON.parse(saved);
        hintsRemaining = { ...hintsRemaining, ...loaded };
    }
}

function saveAllHints() {
    localStorage.setItem(HINTS_STORAGE_KEY, JSON.stringify(hintsRemaining));
}

function getCurrentHints() {
    return hintsRemaining[currentLanguage];
}

function setCurrentHints(value) {
    hintsRemaining[currentLanguage] = value;
    saveAllHints();
    updateHintDisplay();
}
...
function updateHintDisplay() {
    const remaining = getCurrentHints();
...
function updateLanguageBadge() {
    ...
    languageBadge.textContent = info.name;
...
function selectLanguage(language) {
    ...
    const hintsLeft = getCurrentHints();
    
    if (currentQuestionIndex >= totalQuestions) {
        showMessage(`${languageInfo[language].name} is completed! Click Reset Game to play again.`, 'success');
    } else {
        showMessage(`Welcome to ${languageInfo[language].name}! ${hintsLeft} hints left for this language. Starting from Question ${questionNum}/${totalQuestions}`, 'success');
    }
}
...
function changeLevel() {
    ...
    if (currentQuestionIndex >= totalQuestions) {
        ...
        showMessage(`Switched to ${levelText}! Continuing from Question ${questionNum}/${totalQuestions} | ${getCurrentPoints()} pts per question | ${hintsLeft} hints left`, 'success');
    }
}
...
function showHint() {
    ...
    if (currentHints <= 0) {
        showMessage(`You have used all 3 hints for ${languageInfo[currentLanguage].name}! No more hints available for this language.`, 'error');
        return;
    }
    
    const question = currentQuestions[currentQuestionIndex];
    const hintsUsedTotal = TOTAL_HINTS - currentHints;
    ...
    // Decrease hints for current language
    setCurrentHints(currentHints - 1);
    
    showMessage(`💡 Hint used! ${getCurrentHints()} hint(s) remaining for ${languageInfo[currentLanguage].name}.`, 'success');
...
function resetGame() {
    ...
    updateScoreDisplay();
    updateHintDisplay();
    showMessage('Game reset! All progress cleared. All languages have 3 hints again!', 'success');
}
...
function viewHints() {
    console.log(`Hints remaining: Java: ${hintsRemaining.java}, C#: ${hintsRemaining.csharp}, Web: ${hintsRemaining.web}`);
